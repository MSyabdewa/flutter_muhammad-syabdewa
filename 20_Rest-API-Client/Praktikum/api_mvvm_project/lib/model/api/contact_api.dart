...
class ContactApi {
  ...
  Future<List<ContactModel>> getContact() async {
    ...
    //Untuk jawaban soal prioritas 1 nomor 2
    ...
    for (int i = 0; i < contactList.length; i++) {
      print(
          "Contact List dari GET API : id = ${contactList[i].id}, name = ${contactList[i].name}, phone = ${contactList[i].phone}");
    }
  ...
  Future<ContactModel> postContact(ContactModel contactData) async {
    ...
    print(
        "Contact Item dari POST API : id = ${contactItem.id}, name = ${contactItem.name}, phone = ${contactItem.phone}");
