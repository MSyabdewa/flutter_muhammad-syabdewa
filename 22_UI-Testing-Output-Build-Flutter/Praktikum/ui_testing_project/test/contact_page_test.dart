...
void main() {
  testWidgets('Title di appbar harus berjudul Contacts',
  ...
  testWidgets('Menampilkan drawer ketika icon ditekan/slide',
      (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: ContactPage()));
  ...
  testWidgets('Menampilkan Header Contact', (WidgetTester tester) async {
    ...
    expect(
        find.text("Contacts with a Twist : \nPersonalize, Customize, Connect!"),
        findsOneWidget);
  });

  testWidgets('Menampilkan Kondisi awal dari form',
      (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: ContactPage()));
    ...
    expect(
        find.byWidgetPredicate((widget) =>
            widget is TextField &&
            widget.decoration?.hintText == "Insert Your Name"),
        findsOneWidget);
    expect(
        find.byWidgetPredicate((widget) =>
            widget is TextField && widget.decoration?.hintText == "+62 ...."),
        findsOneWidget);
    expect(
        find.byWidgetPredicate((widget) =>
            widget is Container && widget.color == const Color(0xFF21005D)),
        findsOneWidget);
    expect(find.text(DateFormat('d MMMM yyyy').format(DateTime.now())),
        findsOneWidget);
    expect(find.widgetWithText(ElevatedButton, "Submit"), findsOneWidget);
    expect(find.text("List Contacts"), findsOneWidget);
  });

  testWidgets('Menampilkan errortext textfield yang sesuai',
      (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: ContactPage()));

    var nameTextField = find.byWidgetPredicate((widget) =>
        widget is TextField &&
        widget.decoration?.hintText == "Insert Your Name");
    var numberTextField = find.byWidgetPredicate((widget) =>
        widget is TextField && widget.decoration?.hintText == "+62 ....");

    await tester.enterText(nameTextField, "Dewa1");
    await tester.pumpAndSettle();
    expect(find.text("Nama minimal terdiri dari 2 kata"), findsOneWidget);

    await tester.enterText(nameTextField, "syab dewa");
    await tester.pumpAndSettle();
    expect(
        find.text("Setiap kata harus diawali dengan huruf kapital",
            skipOffstage: true),
        findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(
        find.text("Nama tidak boleh mengandung angka/karakter khusus",
            skipOffstage: true),
        findsOneWidget);
    ...
    await tester.pumpAndSettle();
    expect(
        find.text("Nomor telepon harus terdiri dari angka saja",
            skipOffstage: true),
        findsOneWidget);

    await tester.enterText(numberTextField, "9849849292");
    await tester.pumpAndSettle();
    expect(find.text("Nomor telepon harus diawali angka 0", skipOffstage: true),
        findsOneWidget);

    await tester.enterText(numberTextField, "01234567899999999999999");
    await tester.pumpAndSettle();
    expect(
        find.text("Nomor telepon harus terdiri dari 8-15 digit",
            skipOffstage: true),
        findsOneWidget);
  });

  testWidgets('Mensubmit dan menampilkan data yang sudah diinput',
      (WidgetTester tester) async {
    await tester.pumpWidget(const MaterialApp(home: ContactPage()));
    ...
    await tester.dragUntilVisible(find.widgetWithText(ElevatedButton, "Submit"),
        find.byKey(const ValueKey('ScrollKey')), const Offset(0, 200));

    await tester.tap(find.widgetWithText(ElevatedButton, "Submit"));
    await tester.pumpAndSettle();

    expect(find.widgetWithText(ListTile, "Syab Dewa"), findsOneWidget);
    expect(find.widgetWithText(ListTile, "S"), findsOneWidget);
    expect(
        find.byWidgetPredicate((widget) =>
            widget is CircleAvatar &&
            widget.backgroundColor == const Color(0xFF21005D)),
        findsOneWidget);
    expect(find.widgetWithText(ListTile, "0123456789"), findsOneWidget);
    expect(
        find.widgetWithText(ListTile,
            "Birth : ${DateFormat('d MMMM yyyy').format(DateTime.now())}"),
        findsOneWidget);
